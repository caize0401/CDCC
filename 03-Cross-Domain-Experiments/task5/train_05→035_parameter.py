#!/usr/bin/env python3
"""
Domain Adversarial Training with MMD and Adversarial Loss for v3 Model
基于MMD损失和对抗损失的域适应训练脚本（四重损失）
"""
import os
import sys
import argparse
...
def train_one_epoch_mmd_adv(model, train_loader, optimizer, device, epoch, epochs,
                           lambda_mmd=1.0, lambda_aux=0.3, lambda_adv=1e-6):
    """训练一个epoch（对齐train_v3，仅额外加入极小权重的对抗损失）"""
    model.train()
...
@torch.no_grad()
def evaluate_mmd_adv(model, test_loader, device):
    """评估模型（MMD + 对抗版本）"""
    model.eval()
...
def run_mmd_adv_training(source_data1, source_data2, target_data1, target_data2, 
                        model_name: str, source_domain: str, target_domain: str,
                        epochs: int = 100, batch_size: int = 64, lr: float = 0.001,
                        lambda_mmd: float = 1.0, lambda_aux: float = 0.3, lambda_adv: float = 0.0,
                        exp_dir: Path = None):
    """运行MMD + 对抗域适应训练"""
    
    print(f"开始MMD + 对抗域适应训练: {source_domain} → {target_domain}")
    print(f"损失权重 - MMD: {lambda_mmd}, 辅助: {lambda_aux}, 对抗: {lambda_adv}")
    ...
    for epoch in range(1, epochs + 1):
        # 训练（与 train_v3 对齐，返回 train_loss 与 train_label_acc）
        train_loss, train_label_acc = train_one_epoch_mmd_adv(
            model, train_loader, optimizer, device, epoch, epochs,
            lambda_mmd=lambda_mmd, lambda_aux=lambda_aux, lambda_adv=lambda_adv
        )
        
        # 评估
        test_loss, test_metrics, test_label_targets, test_label_preds = evaluate_mmd_adv(
        ...
        print(f"Epoch {epoch:3d}/{epochs} | Train: loss={train_loss:.4f} label_acc={train_label_acc:.4f} | "
              ...
              f"λ_mmd={lambda_mmd:.3f} λ_aux={lambda_aux:.3f} λ_adv={lambda_adv:.3f}")
    ...
    final_test_loss, final_test_metrics, final_test_label_targets, final_test_label_preds = evaluate_mmd_adv(
        model, test_loader, device
    )
    
    print(f"最佳模型在目标域({target_domain})上的最终结果:")
    ...
    detailed_test_metrics = {
        ...
        'test_f1_score': final_test_metrics['f1_score'],
        'epochs': epochs,
    ...
    plt.title(f'Confusion Matrix - {source_domain} → {target_domain}')
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='Domain Adversarial Training with MMD Loss (open version)')
    ...
    parser.add_argument('--model', type=str, default='domain_adversarial_fusion_v3',
                       help='Model name')
...
def main():
    ...
    print("=" * 60)
    print("MMD域对抗网络训练（open版：含极小权重对抗项）")
    print("=" * 60)
    ...
    print("=" * 60)
    print("注意：与 train_v3 一致，仅在训练损失中加入极小权重的对抗项（影响≈0）")
    print("=" * 60)
    ...
    try:
        ...
        # 运行训练
        best_f1, metrics = run_mmd_adv_training(

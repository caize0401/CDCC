"""
Transformer域迁移实验
基于task3的Transformer v4架构，进行0.5→0.35的域迁移实验
只使用data1（原始曲线数据）
"""
import os
import random
...
def run_transformer_domain_transfer():
    ...
    print("=" * 60)
    print("Transformer域迁移实验 (0.5→0.35)")
    print("=" * 60)
    ...
    # 获取训练和测试数据
    df_train = datasets[dataset_type]['05']  # 源域：0.5mm
    df_test = datasets[dataset_type]['035']  # 目标域：0.35mm
    
    print(f"训练集（源域0.5mm）: {len(df_train)} 样本")
    print(f"测试集（目标域0.35mm）: {len(df_test)} 样本")

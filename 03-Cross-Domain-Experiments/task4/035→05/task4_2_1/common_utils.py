"""
Task4_2_1 双路径输入数据处理工具函数
基于task2_3的common_utils.py，适配task4_2_1的实验需求
"""
import os
import sys
import pandas as pd
...
def load_datasets():
    ...
    data2_035_path = datasets_dir / 'data2' / 'features_035.pkl'
    data2_05_path = datasets_dir / 'data2' / 'features_05.pkl'
    
    with open(data2_035_path, 'rb') as f:
        data2_035 = pickle.load(f)
    with open(data2_05_path, 'rb') as f:
...
def align_and_prepare_dual_path(data1, data2):
    ...
    # 提取特征数据
    feature_cols = [col for col in data2.columns if col not in ['CrimpID', 'Sub_label_encoded', 'Wire_cross-section_conductor', 'Main_label_string', 'Sub_label_string', 'Main-label_encoded', 'Binary_label_encoded', 'CFM_label_encoded']]
    features = data2[feature_cols].values
    feature_labels = data2['Sub_label_encoded'].values
